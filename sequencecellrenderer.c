...
static int gapCoord(MSP *msp, int qIdx, int numFrames, Strand strand)
{
  int result = -1 ;
  ...
  int qMax = qForward ? msp->qend : msp->qstart;
  
  if (qIdx < qMin || qIdx > qMax)
    {
      result = 0;
    }
  else
...
//      BOOL rightToLeft = (qForward && strand == REVERSE_STRAND) || (!qForward && strand == FORWARD_STRAND); 
      ...
      if (!gaps || arrayMax(gaps) < 1)
	{
	  ...
	  int offset = (qIdx - qMin)/numFrames ;
...
sequence_cell_renderer_render (GtkCellRenderer *cell,
                                      ...
                                      guint            flags)
{
//  static int i = 0; ++i; printf("render%d\n", i);
  
//  printf("renderer->width = %d, cell_area->width = %d\n", cell->width, background_area->width);
