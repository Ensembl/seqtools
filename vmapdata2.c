 ...
 * Exported functions:
 * HISTORY:
 * Last edited: Mar  4 10:24 2002 (edgrif)
 * Created: Tue Nov 30 18:42:30 1993 (rd)
 * CVS info:   $Id: vmapdata2.c,v 1.1 2009-11-03 18:28:23 edgrif Exp $
 *-------------------------------------------------------------------
 */
...
void vMapMakeDbn (int box)
{
  ...
  if (box != look->activeBox)
    vMapSelect (look, box) ;
  vDbnSeg = seg ;
  mapColSetByName (look->map, "Likelihood", TRUE) ;
...
static void clearMapData (void)
{
  ...
  mapColSetByName (look->map, "Likelihood", FALSE) ;
  myMapDraw (look,0) ;
}
