 ...
 *  acedb
 *
 *  Created by Gemma Barson on 31/08/2010.
                                        ...
                                        const int whitePoint)
{
  ...
  g_signal_connect_swapped(G_OBJECT(quitButton), "pressed", G_CALLBACK(onCloseGreyramp), greyramp); 
...
GtkWidget* createGreyrampTool(DotterContext *dc, const int whitePointIn, const int blackPointIn, const gboolean swapValues)
{
  ...
  gtk_window_set_title(GTK_WINDOW(greyrampTool), "Greyramp Tool");
