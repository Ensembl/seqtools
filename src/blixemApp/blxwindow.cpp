  ...
  if (newWidth != columnInfo->width)
    {
      ...
      int maxWidth = 300;
      gbtools::getMonitorSize(blxWindow, &maxWidth, NULL);
...
//                                           BLX_RESPONSE_RESET,
      ...
      gbtools::getMonitorSizeFraction(dialog, 0.33, 0.33, &width, &height);
      gtk_window_set_default_size(GTK_WINDOW(dialog), width, height);
...
static void setStyleProperties(GtkWidget *widget, char *windowColor)
{
  ...
  int width = 300, height = 200;
  gbtools::getMonitorSizeFraction(widget, DEFAULT_WINDOW_WIDTH_FRACTION, DEFAULT_WINDOW_HEIGHT_FRACTION,
                                  &width, &height);
