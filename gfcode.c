 ...
 *
 * HISTORY:
 * Last edited: Aug 28 09:17 2008 (edgrif)
 * Created: Sun Aug 16 23:19:45 1992 (rd)
 * CVS info:   $Id: gfcode.c,v 1.1 2009-11-03 18:28:23 edgrif Exp $
 *-------------------------------------------------------------------
 */
...
static char *seqName_G = NULL ;

char ident[] = "@(#)standalone genefeatures version 1.1.1, compiled on: "  __TIME__ " " __DATE__ ;
...
*/
	      ...
	      table->logNucFreqs[classOffset+i][j] = lRatio(nCounts,siteSum,selfSample);

	      if (nCounts)
		table->nucFreqs[classOffset+i][j] = nCounts / siteSum;
	      else
		{               /* nCounts == 0 */
		  for (m = 0; m < table->numForced; m++) 
		    if (table->forcedPos[m] - table->startOff == j)
...
static float lRatio (float nCounts, float nTotal, int selfSample)
{
  ...
  return (tPower(nCounts) - tPower(nTotal) + log10((nCounts + 1.0)/(nTotal + 1.0)));
}
...
static GfTableVec *geneFinderInit (char *filename)
{
  ...
  GfTableVec *tableVec ;
  GfTable *table ;
  char fileName[MAXSTRLEN], lastFileName[MAXSTRLEN] ;
  int i,c;
...
void usage (void)
{
  ...
  fprintf (stdout, "  -h(elp)      : print this message\n");
  fprintf (stdout, "  -v(ersion)   : print genefeatures version info\n");
...
static void fMapAddGfSite (int type, int pos, float score, BOOL comp) 
{
  ...
  printf ("%s\tGenefinder\t%s\t%d\t%d\t%.4f\t%c\t%c\n", 
	  seqName_G, featName[idx], start, end, 
	  score, comp ? '-' : '+', '.') ;
}
...
static void fMapAddGfCodingSeg (int pos1, int pos2, float score, BOOL comp)
{
  ...
  printf ("%s\t%s\t%s\t%d\t%d\t%.4f\t%c\t%c\n", 
	  seqName_G, "Genefinder", "coding_seg", pos1, pos2, 
	  score, comp ? '-' : '+', '0') ;
}
...
int main (int argc, char **argv)
{
  ...
  if (argc && (!strcmp (*argv, "-v") || !strcmp (*argv, "-version")))
    {
      printf("\n%s\n\n", ident+4);
      exit(0);
    }
  else if (argc && (!strcmp (*argv, "-h") || !strcmp (*argv, "-help")))
  {
    usage();
  }
  ...
  if (!readSequence (seqFile, dna2textConv, &seq, &seqName_G, 0, &len))
    {
